...
public class HandColorOverlayer : MonoBehaviour 
{
	...
	private void OverlayJoint(long userId, int jointIndex, Transform overlayObj, Rect backgroundRect)
	{
        ...
        //	}
        //}
        if (KinectManager.Instance.GetJointPosColorOverlay(userId, jointIndex, foregroundCamera, backgroundRect)!=Vector3.zero)
	    overlayObj.position =  KinectManager.Instance.GetJointPosColorOverlay(userId, jointIndex, foregroundCamera, backgroundRect);
   //     manager.GetLeftHandState(userid);
	}
